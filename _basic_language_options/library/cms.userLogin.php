...
class userLogin extends systemConfig{
    ...
    //#################################################################
	function getLanguageOptions($web_root, $ccl){
		//CONNECT TO DATABASE
		$connector = new dbConnector();
		...
		while($row	= $connector->fetchArray($result)){
			...
			if($ccl == $languageCode){
				$txt.= '<tr>
					<td width="5%" align="center"><img src="'.$web_root.'images/flags/'.$languageFlag.'" alt="'.$languageName.'" height="25" width="25"/></td>
					<td width="82%">'.$languageName.' <strong>(Selected)</strong></td>
	                <td width="12%" align="center"> - </td>
	            </tr>';
			}else{
				$txt.= '<tr>
	                <td width="5%" align="center"><img src="'.$web_root.'images/flags/'.$languageFlag.'" alt="'.$languageName.'" height="25" width="25"/></td>
	                <td width="82%">'.$languageName.'</td>
	                <td width="12%" align="center">
	                    <a href="'.$languageCode.'" title="Change to this Language" class="change-lang">Change</a>
	                </td>
	            </tr>';
			}
		}
    ...
    //#################################################################
	function getCurrentLanguageOptions($web_root, $ccl){
		//CONNECT TO DATABASE
		$connector = new dbConnector();
		...
		$result	= $connector->query("SELECT * FROM website_languages WHERE languageCode = ? LIMIT 0,1", array($ccl));
		while($row	= $connector->fetchArray($result)){
			//SET VARIABLES
			$languageName	= $row['languageName'];
			$languageFlag	= $row['languageFlag'];
			...
			//GENERATE OUTPUT
			$txt.= '<div class="language-flag"><img src="'.$web_root.'images/flags/'.$languageFlag.'" alt=""/></div>
    ...
    //#################################################################
	function validateLanguageValue($languageCode){
		//CONNECT TO DATABASE
		$connector = new dbConnector();

		//GET LANGUAGE
		$result	= $connector->query("SELECT * FROM website_languages WHERE languageCode = ? AND deletedBy = ? LIMIT 0,1", array($languageCode, 0));
		$total	= $connector->numResults($result);
    ...
    //#################################################################
	function createLanguageCookie($languageCode){
		...
		//SET VARIABLES
		$cookieSessionName = 'ccl';

		//ENCRYPT COOKIE
		$languageCode	= $encryptDecrypt->encrypt($languageCode, ENCRYPTION_KEY);

		//DESTROY COOKIE
		unset($_COOKIE[$cookieSessionName]);

		//SET COOKIE
		setcookie($cookieSessionName, $languageCode, time() + (86400 * 30), '/', '', false, true);
	}

	//#################################################################
    // CREATE LANGUAGE SESSION
    //#################################################################
	function createLanguageSession($languageCode){
		//SET VARIABLES
		$cookieSessionName = 'ccl';

		//SET SESSION
		$_SESSION[$cookieSessionName] = $languageCode;
	}

	//#################################################################
    // DESTROY LANGUAGE COOKIE
    //#################################################################
	function destroyLanguageCookie(){
		//SET VARIABLES
		$cookieSessionName = 'ccl';
	...
	//#################################################################
    // DESTROY LANGUAGE SESSION
    //#################################################################
	function destroyLanguageSession(){
		//SET VARIABLES
		$cookieSessionName = 'ccl';

		//DESTROY SESSION
		unset($_SESSION[$cookieSessionName]);
	}

	//#################################################################
    // CHECK IF LANGUAGE COOKIE AND SESSION ARE CORRECT
    //#################################################################
	function checkLanguageCookieSession($cookieSessionName){
		//CONNECT TO DATABASE
		$connector = new dbConnector();
		$encryptDecrypt = new encryptDecrypt();
		...
		$cookieValue	= $encryptDecrypt->decrypt($_COOKIE[$cookieSessionName], ENCRYPTION_KEY);
		$sessionValue	= $_SESSION[$cookieSessionName];

		//GET DEFAULT LANGUAGE
		$result	= $connector->query("SELECT languageCode FROM website_languages WHERE defaultLanguage = ? LIMIT 0,1", array(1));
		$row	= $connector->fetchArray($result);
		$languageCode	= $row['languageCode'];

		//CHECK IF COOKIE AND SESSION ARE EMPTY
		if($cookieValue == '' && $sessionValue == ''){

			//CREATE LANGUAGE COOKIE & SESSION
			$this->createLanguageCookie($languageCode);
			$this->createLanguageSession($languageCode);
		...
		elseif($cookieValue == ''){
			...
			$total	= $this->validateLanguageValue($sessionValue);

			//NOT VALID
			if($total == 0){
				//DESTROY COOKIE & SESSION
				$this->destroyLanguageCookie();
				$this->destroyLanguageSession();

				//RECREATE COOKIE & SESSION
				$this->createLanguageCookie($languageCode);
				$this->createLanguageSession($languageCode);
			...
			//VALID
				...
				//RECREATE COOKIE & SESSION
				$this->createLanguageCookie($sessionValue);
				$this->createLanguageSession($sessionValue);
		...
		elseif($sessionValue == ''){
			...
			$total	= $this->validateLanguageValue($cookieValue);
			...
			//VALID
				...
				//RECREATE COOKIE & SESSION
				$this->createLanguageCookie($cookieValue);
				$this->createLanguageSession($cookieValue);
		...
		//CHECK IF COOKIE AND SESSION
			...
			//VALUES ARE THE SAME
				...
				//RECREATE COOKIE TO LAST 30 DAYS
				else{
					//RECREATE COOKIE
					$this->createLanguageCookie($cookieValue);
				}
...
//CHECK LANGUAGE COOKIE AND SESSION
$userLogin->checkLanguageCookieSession('ccl');

...
<?php if($moduleID == 100){ ?>
	<script type="text/javascript" language="javascript">
        /*REMOVE STAFF MEMBER*/
        function deleteLanguage(bos) {
            var msg = "Are you sure you want to de-activate the selected Language? You can always re-activate it later.";
            if (confirm(msg)) {
                eval('document.delete_language'+bos+'.method = "post"');
                eval('document.delete_language'+bos+'.action = "<?php echo $_SERVER['PHP_SELF'] ?>"');
                eval('document.delete_language'+bos+'.submit()');
            } else {
                return;
            }
        }
	...
	<script type="text/javascript" language="javascript">
        /*RECOVER LANGUAGE*/
        function recoverLanguage(bos) {
            var msg = "Are you sure you want to activate the selected Language?";
            if (confirm(msg)) {
                eval('document.recover_language'+bos+'.method = "post"');
                eval('document.recover_language'+bos+'.action = "<?php echo $_SERVER['PHP_SELF'] ?>"');
                eval('document.recover_language'+bos+'.submit()');
            } else {
                return;
            }
        }
...
<div class="wrapper">
    ...
    <div class="top-navigation">
		...
		<div id="changeLanguage" class="language-selector" title="Change Language">
			<?php echo $userLogin->getCurrentLanguageOptions($web_root, $_SESSION['ccl']); ?>
		</div>
        <div class="clear"></div>

...
//GET URL VARIABLE
if(isset($_POST['vacID'])){$vacID = $_POST['vacID'];}else{$vacID = $_GET['vacID'];}
...
//REDIRECT PAGE
if($vacID != ''){
	//CHECK $vacID INSIDE DATABASE
	if($vacancyManager->checkVacancyDatabase($vacID) == 'not found'){
...
//SET BREADCRUMBS
$breadcrumbs = '<a href="'.$cms_root.'" title="Dashboard">Dashboard</a> | <a href="'.$cms_root.'vacancy-manager/" title="Vacancy Manager">Vacancy Manager</a> | <span class="current">Edit Vacancy</span>';
...
?>
    ...
    <div class="content-holder">
    	...
    	<h1 align="center">Edit Vacancy</h1>
        <div class="intro">
        	<p>This is the <b>Edit Vacancy</b> page. This page will allow you to edit the current Vacancy.</p>
        </div>

        <div class="left-column">
            <div class="module-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Edit Vacancy</div>
                <div class="clear"></div>
                <div class="module-holder-intro">
                	Please change all the required field below to edit the Vacancy. <span class="required">(*) Required</span><br />
                	Once you are done click on <b>Edit Vacancy</b> to edit the Vacancy.
                </div>
                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                    <div class="module-form-holder">
                        ...
                        <input type="hidden" name="modifiedNumber" value="<?php echo $vacancyManager->getVacancyInfo($vacID, 'modifiedNumber')+1;?>"/>

						<div class="module-form-titles"><span class="required">*</span> Vacancy Title:</div>
						<input type="text" name="vacancy-title" placeholder="Vacancy Title" value="<?php if($_POST['vacancy-title'] != ''){echo $_POST['vacancy-title'];}else{echo $vacancyManager->getVacancyInfo($vacID, 'jobTitle');}?>" maxlength="150" />
						...
						<select name="owner">
							...
							<?php
								if($_POST['owner'] != ''){
									...
									$owner = $vacancyManager->getVacancyInfo($vacID, 'vacOwnerID');
								}

								echo $vacancyManager->getOwners($owner);
							?>
						</select>
						...
						<select name="type">
							...
							<?php
								if($_POST['type'] != ''){
									...
									$type = $vacancyManager->getVacancyInfo($vacID, 'type');
								}

								echo $vacancyManager->getType($type);
							?>
						</select>
						...
						<select name="city">
							...
							<?php
								if($_POST['city'] != ''){
									...
									$city = $vacancyManager->getVacancyInfo($vacID, 'city');
								}

								echo $vacancyManager->getCity($city);
							?>
						</select>
						...
						<select name="category">
							...
							<?php
								if($_POST['category'] != ''){
									...
									$category = $vacancyManager->getVacancyInfo($vacID, 'category');
								}

								echo $vacancyManager->getCategory($category);
							?>
						</select>
						...
						<select name="salary">
							...
							<?php
								if($_POST['salary'] != ''){
									...
									$salary = $vacancyManager->getVacancyInfo($vacID, 'salary');
								}

								echo $vacancyManager->getSalary($salary);
							?>
						</select>
						<i>Please select the salary.</i>

						<div class="module-form-titles">Vacancy Email:</div>
						<input type="email" name="vacancy-email" placeholder="Vacancy Email" value="<?php if($_POST['vacancy-email'] != ''){echo $_POST['vacancy-email'];}else{echo $vacancyManager->getVacancyInfo($vacID, 'email');}?>" />
						...
						<div class="user-module-holder">
                            <label>
                                <input type="checkbox" name="application-form" <?php if($_POST['application-form'] == 1){echo 'checked="checked"';}elseif($vacancyManager->getVacancyInfo($vacID, 'showForm') == 1){echo 'checked="checked"';}?> value="1">
                        ...
                        <div class="module-date-input">
                            ...
                            <input type="text" name="start-date" id="datepicker" placeholder="Start Date" value="<?php if($_POST['start-date'] != ''){echo $_POST['start-date'];}else{echo $vacancyManager->getVacancyInfo($vacID, 'startDate');}?>">
                            <i>Please supply the start date of the vacancy.</i>
                        </div>

						<div class="module-time-input">
							<div class="module-form-titles">End Date:</div>
                            <input type="text" name="end-date" id="datepicker2" placeholder="End Date" value="<?php if($_POST['end-date'] != ''){echo $_POST['end-date'];}else{echo $vacancyManager->getVacancyInfo($vacID, 'endDate');}?>">
						...
						<textarea name="paragraph" cols="20" rows="5"><?php if($_POST['paragraph'] != ''){echo $_POST['paragraph'];}else{echo $vacancyManager->getVacancyInfo($vacID, 'description');}?></textarea>
					...
					<input type="submit" class="module-form-submit" name="edit_vacancy" title="Edit Vacancy" value="Edit Vacancy" onclick="pleasewait()"/>
	            </form>
            </div>

            <div class="module-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Vacancy Info</div>
                <div class="clear"></div>
                <div class="module-holder-intro">
                	Below you can see some information about the current <b>Vacancy</b>.
                </div>
                	...
                	<table width="100%" class="edit-information-table">
                      <tr>
                        <td width="50%">
                            ...
                            <?php echo $vacancyManager->getUsersName($vacancyManager->getVacancyInfo($vacID, 'creadtedBy'));?>
                       </td>
                        <td width="50%">
                        	<div class="edit-information-table-label"><b>Modified By:</b></div>
                            <?php
                            	if($vacancyManager->getVacancyInfo($vacID, 'modifiedBy') != 0){
									echo $vacancyManager->getUsersName($vacancyManager->getVacancyInfo($vacID, 'modifiedBy'));
								}else{
									echo '-';
								}
							?>
                      ...
                      <tr>
                        <td>
                        	...
                        	<?php echo date("j F Y",strtotime($vacancyManager->getVacancyInfo($vacID, 'createdDate')));?>
                        </td>
                        <td>
                        	<div class="edit-information-table-label"><b>Last Modified Date:</b></div>
                        	<?php
                            	if($vacancyManager->getVacancyInfo($vacID, 'modifiedDate') != 0){
									echo date("j F Y", strtotime($vacancyManager->getVacancyInfo($vacID, 'modifiedDate')));
								}else{
									echo '-';
								}
							?>
                      ...
                      <tr>
                        ...
                        <td>
                        	...
                        	<?php echo $vacancyManager->getVacancyInfo($vacID, 'modifiedNumber');?>
                        </td>
                      </tr>
                    </table>

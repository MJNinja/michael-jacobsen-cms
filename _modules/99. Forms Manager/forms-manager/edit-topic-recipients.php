...
$moduleID = 99;
$colorbox = 1;
$pageTitle = 'Edit Topic Recipient';
...
//REDIRECT PAGE
if($recipientID != '' && $formID != '' && $topicID != ''){
	//CHECK recipientID AND formID INSIDE DATABASE
	if($formsManager->checkTopicRecipientDatabase($recipientID, $formID, $topicID) == 'not found'){
...
?>
    ...
    <div class="content-holder">
    	...
    	<h1 align="center">Edit Topic Recipient - <?php echo $formsManager->getRecipientInfo($recipientID, 'fullname'); ?></h1>
        <div class="intro">
        	<p>This is the <b>Edit Topic Recipient</b> page. This page will allow you to edit the current topic recipient (<?php echo $formsManager->getRecipientInfo($recipientID, 'fullname'); ?>).</p>
        </div>

        <div class="left-column">
        	<!-- BEGIN PARAGRAPH HOLDER-->
            <div class="module-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Edit Topic Recipient</div>
                <div class="clear"></div>
                <div class="module-holder-intro">
                	Please fill out all the required field below to edit the current topic recipient. <span class="required">(*) Required</span><br />
                	Once you are done click on <b>Edit Topic Recipient</b> to edit the current topic recipient.
                </div>
                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                    <div class="module-form-holder">
						...
						<input type="text" name="topic-recipient-name" placeholder="Topic Recipient Name" value="<?php if($_POST['topic-recipient-name'] != ''){echo $_POST['topic-recipient-name'];}else{echo $formsManager->getRecipientInfo($recipientID, 'fullname');}?>" maxlength="150" />
						...
						<input type="text" name="topic-recipient-email" placeholder="Topic Recipient Email" value="<?php if($_POST['topic-recipient-email'] != ''){echo $_POST['topic-recipient-email'];}else{echo $formsManager->getRecipientInfo($recipientID, 'email');}?>" />
                        <i>Please supply the email address of the new topic recipient</i>

                    </div>
					<input type="submit" class="module-form-submit" name="edit_topic_recipient" title="Save Changes" value="Save Changes" onclick="pleasewait()"/>
                </form>
            </div>
            <!-- END PARAGRAPH HOLDER-->

			<div class="module-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Topic Recipient Info</div>
                <div class="clear"></div>
                <div class="module-holder-intro">
                	Below you can see some information about the current <b>Topic Recipient</b>.
                </div>

<?php
//SET DEFAULT VARIABLES
$cms = 1;
$moduleID = 2;
$gallery_upload = 1;
$sequence = 1;
$sequenceTable = 'basic_pages_gallery_content';
$sequenceMainID = 'basicPagesGalleryContentID';
$pageTitle = 'Sequence Gallery';

//GET URL VARIABLE
if(isset($_POST['pageID'])){$pageID = $_POST['pageID'];}else{$pageID = $_GET['pageID'];}
if(isset($_POST['basicPagesGalleryID'])){$basicPagesGalleryID = $_POST['basicPagesGalleryID'];}else{$basicPagesGalleryID = $_GET['basicPagesGalleryID'];}
require_once("../inc/cms-owner-info-inc.php");
require_once("../../library/cms.userLogin.php");
require_once("../../library/class.systemConfig.php");
require_once("../../library/cms.basicContentManager.php");

//CHECK IF USER IS ALLOWED TO ACCESS MODULE
require_once("../inc/cms-user-module-rights-inc.php");

//REDIRECT PAGE
if($pageID != '' && $basicPagesGalleryID != ''){
	//CHECK $pageID INSIDE DATABASE
	if($basicContentManager->checkPageIDDatabase($pageID) == 'not found'){
		header("Location:".$cms_root."basic-content-manager/");
		exit;
	}

    //CHECK $basicPagesGalleryID INSIDE DATABASE
    if($basicContentManager->checkPageGalleryDatabase($basicPagesGalleryID) == 'not found'){
		header("Location:".$cms_root."basic-content-manager/");
		exit;
	}
}else{
	header("Location:".$cms_root."basic-content-manager/");
	exit;
}

//SET BREADCRUMBS
$breadcrumbs = '<a href="'.$cms_root.'" title="Dashboard">Dashboard</a> | <a href="'.$cms_root.'basic-content-manager/" title="Basic Content Manager">Basic Content Manager</a> | <a href="'.$cms_root.'basic-content-manager/manage-pages.php?pageID='.$pageID.'" title="Manage Page Content">Manage Page Content</a> | <span class="current">Sequence Gallery</span>';

require_once("../inc/header-inc.php");
require_once("../inc/navigation-inc.php");
?>

    <!-- BEGIN CONTENT HOLDER -->
    <div class="content-holder">

    	<h1 align="center">Sequence Gallery - <?php echo $basicContentManager->getPageInfo($pageID, 'pageName'); ?></h1>
        <div class="intro">
        	<p>This is the <b>Sequence Gallery</b> page. This page will allow you to sequnce the current gallery (<?php echo $basicContentManager->getPageInfo($pageID, 'pageName'); ?>).</p>
        </div>

        <div class="left-column">
        	<!-- BEGIN GALLERY IMAGE PREVIEW -->
            <div class="module-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Sequence Gallery</div>
                <div class="clear"></div>
                <div class="module-holder-intro"></div>

                <div class="module-form-holder">
                    <div class="module-form-titles">Current Gallery Image(s)</div>
                    <p>
                        Below are all the images currently assigned to this gallery. To change the sequence of the gallery simply click and hold your left mouse button, and drag the images accordingly.
                    </p>

					<div id="sortable">
                    	<?php echo $basicContentManager->getPageGalleryImagesSequencing($basicPagesGalleryID, $web_root);?>
					</div>

                </div>
            </div>
            <!-- END GALLERY IMAGE HOLDER-->

        </div>

        <div class="right-column module-stats-spacing">
            <div class="module-stats-holder">
                <div class="module-holder-name">&nbsp;&nbsp;Module Stats</div>
                <div class="clear"></div>

                <div class="module-stats-container">
                	<?php include_once("../inc/basic-content-stats-inc.php"); ?>
            	</div>
            </div>
        </div>

        <div class="clear"></div>
    </div>
    <!-- END CONTENT HOLDER -->

<?php
require_once("../inc/footer-inc.php");
?>
